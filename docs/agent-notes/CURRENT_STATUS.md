# 現在の状態（最終更新：2026-08-25 (6) by Claude）
...
- `daily-update.yml` は `gemini-3.1-flash-lite` への切り替え後、191件のデータで
  ...
  対応済み。さらに吉野さんから「本当の意味での新着順（日付順）をしっかり実装したい」
  との要望を受け、記事ページ本体から正確な公開日を取得する仕組みを追加し、
  既存データにも遡って適用した（191件中、published_dateなしが187件→98件に減少。
  下記changelog参照）。**次回実行時に、新しく承認された記事の日付が
  `collected_at`頼みではなく実際のページ更新日になっているか確認してほしい。**
- `go-minamata.jp`（観光物産協会）は記事ページ自体に日付表示が無いことを確認済みのため、
  同サイト由来の項目は今後も`collected_at`（確認日）表示のままになる。これを許容するか、
  将来的に別の日付情報源（例えばイベント名や本文からの日付抽出等）を検討するかは未定。
- 本番デプロイ（Pages）に向けたフロー確認

## 直近の変更履歴（簡易、詳細はdecisions-log参照）

- 2026-08-25: [Claude] 吉野さんから「本当の意味での新着順（日付順）をしっかり実装したい」
  との要望。`published_date`が欠落する原因は、新着一覧ページ（`new_list.html`等）側に
  日付が併記されているリンクが少ないためと判明（実データで191件中187件が欠落）。
  一方、`monitor_stock.py`の調査で判明済みの通り、水俣市公式サイト
  （`city.minamata.lg.jp`、防災・スポーツ含む）の記事ページ本体には信頼できる
  `<time datetime="...">`マークアップが存在する。観光物産協会サイト
  （`go-minamata.jp`）は記事ページ自体を確認したが日付表示が一切無いことを実際に
  確認した。対応として、Geminiに「関係あり」と判定された項目のうちpublished_date
  未取得のものに限り（無駄なアクセスを避けるため）、`city.minamata.lg.jp`ドメインの
  記事ページへ改めてアクセスして正確な日付を取得する`fetch_article_published_date()`
  を追加。既存の`life_info.json`にも同じロジックで遡って適用し、180件中89件で
  実際の更新日（2018年〜2026年の実データ）を取得できることを確認、
  published_date欠落を187件→98件に削減した。
- 2026-08-25: [Claude] 吉野さんから「見出しの部分に日時が欲しい」との要望。
  `src/index.html`のフィードカード（`cardHTML`）・関連ニュースカード（`newsCardHTML`）は
  元々`published_date`を表示する枠があったが、`published_date`がない項目（大半）では
  空欄のままだった。`displayDateLabel()`を新設し、`published_date`があればそれを、
  なければ`collected_at`を代わりに表示するようにした（ソート処理と同じフォールバック
  方針）。ただし`collected_at`は実際の公開日ではないため誤解を避ける目的で
  「確認：」という接頭辞を付けて区別している。実データ（公開日あり・なし各1件）で
  表示結果を確認済み（例: 「2026年8月21日」／「確認：2026年8月25日」）。
- 2026-08-25: [Claude] `gemini-3.1-flash-lite`への切り替え後、`daily-update.yml`が
  191件で正常完走。ただし吉野さんから2点指摘があり調査・対応した。
  **(1) 文字化けの再発**: 実データを調べたところ、化けていた12件は全て
  `collected_at`が2026-08-24T21:4x台（文字コード修正が本番で効く前の実行）のものと判明。
  重複排除機能により、一度`_checked_urls.json`に記録されたURLは二度と再取得・再判定
  されない設計のため、修正前に保存された化けたタイトルが恒久的に残り続けていた
  （新規に化けるバグの再発ではなく、過去データの残骸）。titleのみが化けており
  summary等Gemini生成フィールドは無事だったため、`title.encode('latin-1').decode('utf-8')`
  で元のUTF-8バイト列を復元できることを確認し、該当12件のtitleを直接修復した
  （すでに正常なtitleはlatin-1エンコード時点で例外になるため誤爆しない）。
  **(2) 並び順**: `merge_items()`は`published_date`の降順でソートしていたが、
  実データでは191件中187件が`published_date`なし（制度案内ページ等は本文に
  明確な日付表記がなくGeminiも抽出できない）だったため、ソートがほぼ機能せず
  順序が意味をなさなくなっていた。`published_date`がない場合は`collected_at`
  （収集日時）を代わりに使うよう`update_data.py`（新設の`sort_key()`）と
  `src/index.html`のフロントエンド側ソートの両方を修正し、既存の`life_info.json`にも
  同じロジックで再ソートを適用済み。
- 2026-08-25: [Claude] 吉野さんがAI Studioのレート制限画面で、他のモデル
  （`gemini-3.1-flash-lite`等）はRPD使用量0のまま（モデルごとに枠が独立）である
  ことを確認。`gemini-3.5-flash-lite`のRPD枯渇は今日中には解消しない可能性が高いため、
  `scripts/update_data.py`の`GEMINI_MODEL`デフォルトを`gemini-3.1-flash-lite`に変更し、
  今日中の検証を続行できるようにした（RPM15・RPD500は同一）。実エンドポイントに
  無認証でPOSTし、403（認証エラー）が返ることを確認済み（404なら存在しないモデル名だが
  そうではなかった）。
