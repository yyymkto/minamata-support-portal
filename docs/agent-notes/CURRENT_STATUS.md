# 現在の状態（最終更新：2026-08-25 (4) by Claude）
...
- `daily-update.yml` は `gemini-3.1-flash-lite` への切り替え後、191件のデータで
  ...
  一通り実運用で機能することを確認済み）。ただし完走後に吉野さんから
  「並び順がおかしい」「まだ文字化けしている項目がある」の指摘があり、
  いずれも原因判明・対応済み（下記changelog参照）。**次回実行時に、
  並び順が新着っぽく自然に見えるか・文字化けが再発しないか、改めて見てほしい。**
