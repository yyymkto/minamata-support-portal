# 現在の状態（最終更新：2026-08-25 (7) by Claude）
...
- **【試験導入・要評価】** 並び順を「実日付グループを常に確認日グループより上に」という
  二段階ソートに変更した（下記changelog参照）。この方式では、2018年など古い実日付の
  記事でも、確認日のみの真に新しい発見より上位に来る（実データで最古の実日付は
  2018-03-05）。吉野さんの希望で試験的に導入したが、「新着情報」として見たときに
  古い実日付コンテンツが目立ちすぎないか、実際の見た目で評価してほしい。
  違和感があれば、単純な日付混在ソート（旧方式）や、実日付でも一定期間より古いものは
  確認日グループ扱いにする中間案などに戻すことも検討。
