# 現在の状態（最終更新：2026-08-25 by Claude）
...
- **【要対応・最優先】** 2026-08-24中に何度も手動実行を繰り返した結果、
  `gemini-3.5-flash-lite`無料枠の**RPD（1日の上限、AI Studio上は500）を使い切った
  可能性が高い**（134件目以降、リトライしても永久に429が続く挙動を確認。RPM超過なら
  時間経過で回復するはずが全く回復しなかったため）。この場合、日本時間の16-17時頃
  （太平洋時間の深夜0時）にリセットされるまで、どう対処しても解決しない。
  対応として、リトライを使い切ってなお失敗する状態が3件連続したら残りを打ち切り、
  ここまでの結果を保存する「サーキットブレーカー」を追加した（下記changelog参照）ので、
  今後は無駄にジョブを長時間（数時間）粘らせることはなくなるはず。
  **次にRPDがリセットされたタイミングで `daily-update.yml` を実行し、
  文字化け修正・429対応（リトライ＋ペース調整＋判定済みURLスキップ）が
  一通り正しく機能することを確認してほしい。**
  確認ポイント：(1) `life_info.json` のtitleが正しい日本語になること、
  (2) 429がほぼ出ずに最後まで完走すること、(3) 2回目の実行で「判定済みとしてスキップ」
  件数が増えていること。今リポジトリに入っている `life_info.json` は、
  上記対応より前の（文字化け＆429で候補がほぼ全滅した）データのまま。
