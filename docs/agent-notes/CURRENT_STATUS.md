# 現在の状態（最終更新：2026-08-25 (3) by Claude）
...
- **【要対応・最優先】** `gemini-3.5-flash-lite`の無料枠1日500リクエスト上限(RPD)を
  2026-08-24中のテストで使い切ったため（429応答本文で
  `"Quota exceeded for metric: .../generate_content_free_tier_requests, limit: 500,
  model: gemini-3.5-flash-lite"` と確定済み）、吉野さんがAI Studioで確認した
  「モデルごとに枠が完全に独立している」ことを踏まえ、**`GEMINI_MODEL`のデフォルトを
  `gemini-3.1-flash-lite`（同じくRPM15・RPD500だが、この時点で未使用＝枠フル）に
  切り替えて今日中の検証続行を可能にした**。API疎通は403（認証エラー、404ではない）で
  実在するモデルであることを確認済み。あくまで今日のテスト続行のための切り替えであり、
  `gemini-3.5-flash-lite`自体は実運用（1日1回）では枠が足りていたはずなので、
  どちらのモデルを最終的に使うかは吉野さんの判断次第（性能・コストに大きな違いはない想定）。
  **次のdaily-update.yml実行で、今度こそ最後まで完走するか確認してほしい。**
