# 現在の状態（最終更新：2026-08-26 (3) by Claude）
...
- **【方針転換・実装済み】** 「使える制度をさがす」タブについて、吉野さんから
  ...
  タブの新設を参照）。
- **【要確認】** 新設した「一人暮らし・若者支援」タブ（下記changelog参照）は
  実データ・実装ともローカルのjsdomテストで動作確認済みだが、実際のブラウザ・
  本番デプロイでの見た目の確認はまだ。吉野さんに実際の画面を見てもらい、
  カテゴリ分け・掲載内容に違和感がないか確認してほしい。
