...
| 子育て支援制度タブの改装（制度の追加＋情報階層の整理） | Claude Code（設計・データ）→Antigravity（UI） | 進行中。設計書は`design-specs/2026-09-16_child-support-page-renovation.md`。UI（タスク4a, 4b, 5〜9、Antigravity）とデータ（タスク1〜4、Claude Code）は完了・ローカルコミット済み（未push）。**残り：タスク0b（既存制度の説明文の事実化、約22件）と、タスク10（ブラウザでの全体確認、吉野さん）** |
...
- **【追加候補・未着手】** 2026-09-17のカテゴリ全件確認で、設計書の候補10件以外にも
  有力な未掲載ページが見つかった：水俣市不妊治療費助成（kiji0031656）、
  母子家庭等高等職業訓練促進給付金（kiji0034082）、みなまた子育てガイドブック（kiji0033559）。
  掲載するかは吉野さんと相談（制度数は現在36件で、設計書の「40件を超えたら折りたたみか
  検索が必要」の目安に近づいている）。
- **【潜在バグ・未対応】** `src/index.html` の `state` 初期化で
  `localStorage.getItem` を try/catch なしで呼んでいる。ブラウザ設定でサイトデータが
  ブロックされている環境では、アクセス時に例外が出てスクリプト全体が止まる可能性がある。
  改装とは無関係の既存コードのため今回は触っていない。
