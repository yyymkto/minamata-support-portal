...
- 2026-09-22: [Claude Code] イベントタブのスプレッドシート連携を本番で確認（手動実行 run 35646112708）。
  手動登録1件を含む17件が本番の `events.json` に反映。イベントタブ（設計書 `design-specs/2026-09-21_events-tab.md`）は全タスク完了。
- 2026-09-22: [Claude Code] 吉野さんが作成したイベント登録用スプレッドシートの公開CSVを読み込み確認。登録された
  Instagramのリンクに共有者識別用とみられる `stkn` 等のパラメータが付いていたため、`update_events.py` で
  InstagramのURLの `?` 以降を取り除くよう修正。
