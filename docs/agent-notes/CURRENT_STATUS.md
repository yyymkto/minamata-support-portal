...
| イベントタブの新設（若者タブを一時オフにして置き換え） | Claude Code（設計・収集スクリプト）→吉野さん（スプレッドシート準備）→Antigravity（UI） | 設計書`design-specs/2026-09-21_events-tab.md`。Claude担当のタスク1〜3（自動抽出・手動読み込み・CI追加）完了。Antigravity担当のUI（タスク5: 若者タブ一時オフ、タスク6: イベントタブ追加、タスク7: 一覧描画・グループ分け、タスク8: 表示確認・テスト）の実装・検証完了。2026-09-22にClaude Codeがレビューし1点修正（「詳細で詳細」表示）、吉野さんの依頼でpush・本番反映済み。残り：タスク4（スプレッドシート準備、吉野さん）と、手動実行run 35635663082の「イベント情報を更新」ステップのログで「=== 完了 ===」の確認（吉野さん） |
...
- 並び順の二段階ソート（実日付グループを確認日グループより常に上に）は、
  吉野さんの実際の見た目確認により「良い感じ」と評価済み。このまま採用継続でよい。
- **【方針転換・実装済み】** 「使える制度をさがす」タブについて、吉野さんから
  「水俣市内在住者向け」という前提を外し、「水俣に来たらこんな支援が受けられる」を
  市内在住者・市外の人の両方に自然に伝わるようにしたい、との要望があった。
  ただし「〜たら」「〜ますよ」といった誘導的・勧誘的な言い回しは避けたいとのことで、
  「制度を探す（あなた基準）」ではなく「制度そのものを紹介する（中立的な事実描写）」
  という最小限の言い換えに留めた（タブ名「使える制度をさがす」→「子育て支援制度」、
  見出し・リード文も同様に「〜が使える制度」→「水俣市の子育て支援制度」に変更。
  詳細は下記changelog参照）。個別の制度カードの説明文（`child_support_base.json`の
  `description`等）は元々中立的な紹介文だったため変更していない。
  → 移住者向けの切り口を別タブとして追加する方向で決着（下記「一人暮らし・若者支援」
  タブの新設を参照）。
- 「一人暮らし・若者支援」タブは吉野さんに実際の画面を見てもらい、カテゴリ分け・
  掲載内容自体には特に指摘なし。ただしストック情報全般（子育て支援制度・
  一人暮らし若者支援の両タブ）に表示していた「関連する最新のお知らせ」機能
  （困りごとのトピックタグと一致する新着記事を最大2件表示）について
  「あまりピンと来ない」との指摘があり、2026-08-27に削除した（下記changelog参照）。
- 本番デプロイ（Pages）に向けたフロー確認
- **【本番確認待ち】** 制度カードの共有リンク（2026-09-21 push 済み）。LINEプッシュ通知は見送り、
  「制度カードごとに共有ボタンを付け、受け取ったリンクがその制度を開く」方式に決定
  （`decisions-log/2026-09-18_share-link-instead-of-push.md`）。設計書は
  `design-specs/2026-09-18_share-link.md`。Antigravityによりタスク1〜6の実装・検証完了、
  2026-09-21にClaude Codeがレビューし3点を修正（設計書の疑問点欄に記録）。
  push後、本番URLの共有リンクをLINEに送ってプレビューと開き方を確認する。
- **【次の設計課題】** 子育て支援制度タブの制度数が39件になり、設計書
  （`design-specs/2026-09-16_child-support-page-renovation.md`「なぜ折りたたみ・検索を今回入れないか」）
  で想定した「40件を超えたら折りたたみか検索が必要」の目安に達した。
  これ以上追加する前に、折りたたみ・検索の設計書を起こすか吉野さんと相談する。

## 直近の変更履歴（簡易、詳細はdecisions-log参照）

- 2026-09-22: [Claude Code] イベントタブUI（Antigravity実装）をレビューし、情報元が「詳細」のときにリンクが
  「詳細で詳細」となる問題を修正。吉野さんの依頼でpushし、本番にイベントタブが表示され若者タブが非表示になったことを確認。
...
- 2026-09-21: [Claude Code] 共有リンクの実装をレビュー。jsdomで実HTML・実データの
  ...
  (2) 全体像リンクでも「共有されたリンクを開いたため」と出る文言（設計書の指定漏れ）を
  「この制度を表示するため、絞り込みを一時的に解除しています。」に変更、
  (3) データ読み込み失敗時に「制度が見つかりませんでした」が誤って出る問題を修正。
- 2026-09-18: [Antigravity] 制度カードの共有リンク（設計書 `design-specs/2026-09-18_share-link.md`）のタスク1〜6を `src/index.html` に実装完了。
  (1) タスク6（OGPタグ）：`<head>` に `og:type`, `og:site_name`, `og:title`, `og:description`, `og:url`, `twitter:card` を追加（「非公式」表記を保持）。
  (2) タスク1・2（共有ボタンと共有処理）：`serviceCardHTML` の最下部帯（最終確認日行）の右端に共有ボタンを配置。`navigator.share`（キャンセル時無視）およびクリップボードコピー（成功時「リンクをコピーしました」トーストを2秒表示、失敗時URLプロンプト）を実装。
  (3) タスク3・4・5（共有リンク展開と例外系）：URLハッシュ（`#base-{id}`）から子育て支援・若者支援の各タブへの自動切り替えとスムーズスクロールを実装。プロフィール絞り込みで対象カードが隠れる場合は一時的に解除してカードを表示し、「共有されたリンクを開いたため、絞り込みを解除して表示しています。［絞り込みに戻す］」通知を設置（localStorageは不変）。存在しないIDの場合はフィードタブのまま案内を一行表示。
  Node.js仮想環境でのシナリオ自動テスト（5ケース）を実施し、すべて正常動作することを確認済み。
- 2026-09-18: [Claude Code] 吉野さんとの相談で、希望者へのLINEプッシュ通知は見送り、
  制度カードごとの共有リンクを先に作ることを決定。設計書
  `design-specs/2026-09-18_share-link.md` と決定記録
  `decisions-log/2026-09-18_share-link-instead-of-push.md` を作成。
  主眼は共有ボタンそのものより、受け取ったリンクが正しい制度を開くようにすること
  （現在はサイトを開くと必ずフィードタブが表示されるため、`#base-...` 付きのURLを
  送っても受け取った人にはその制度が見えない）。実装はAntigravity担当。
- 2026-09-17: [Claude Code] 吉野さんの依頼で、前回の報告で残した3件を実施。
  (1) **`localStorage` の潜在バグを修正**：プロフィールの読み書きを `loadProfile` /
  `saveProfile` にまとめて try/catch で包んだ。サイトデータがブロックされた環境や、
  保存値が壊れている場合でもスクリプト全体が止まらない（Nodeで4パターン確認）。
  (2) **タスク0b完了**：既存制度23件の説明文・条件・申請方法を、公式ページ本文と
  お問い合わせ先（ページ下部の担当課）に合わせて書き直した。事実の誤りとして、
  窓口が「福祉課」になっていた5件（児童扶養手当・ひとり親医療費助成・福祉資金貸付・
  女性相談・里親）を実際の担当のこども子育て課に訂正、母子健康手帳から移行済みの
  「出産・子育て応援給付金」の記載を削除、日常生活用具から対象外の「酸素発生器」を削除、
  last_verified に調査日が入っていた3件をページ更新日に訂正した。
  (3) **候補外3件を追加**：不妊治療費助成・高等職業訓練促進給付金・みなまた子育てガイドブック。
  新しい困りごとは作らず既存の入口に紐づけた。制度数は39件。
  検証：描画チェック（壊れたリンク無し・全39件表示・絞り込み追随）、監視スクリプトの読み込み、
  評価・呼びかけ語（「ぜひ」「安心して」等）が説明文に無いことを確認。
  コミットは push 前のリベースで `e822792` 等に付け替わった（2026-09-17 push 済み）。
- 2026-09-17: [Claude Code] 子育てタブ改装のデータ側（設計書タスク1〜4）を完了。
  `child_support_base.json` は28件→36件、`concern_mapping.json` の困りごとは16件→20件。
  (1) タスク1・2：追加候補10件の実ページ本文を1件ずつ取得して確認し、8件を追加
  （定期予防接種・HPVワクチン・市外/県外での予防接種・市奨学金（貸付型）・
  松本眞一同朋奨学金（給付型）・こどもセンター・発達支援パンフレット・民法改正の解説）。
  療育センター（2015年更新。所在地・電話が現行パンフレットでは「児童発達支援センターにこにこ」
  として載っている）と、こども園の学童クラブ（2019年更新。同じ内容が学童募集ページに
  最新の連絡先つきで載っている）は掲載を見送った。
  (2) タスク3：困りごと4件（予防接種・奨学金・遊び場・発達）を追加。重複を避けるため
  既存の「健診や予防接種」の見出しから予防接種を外し、存在しない base_id への参照
  （birth-childcare-support-grant）を除去した。
  (3) タスク4：病児保育を常設ページ（kiji003384）へ差し替え。常設ページは土曜利用可の
  ままだが、2026-01-11から月〜金に変更済みのため条件欄で注記。児童手当・学童は
  市サイトのカテゴリ全記事を確認して常設ページが無いことを確かめ、URLは維持。
  (4) **児童手当の説明文が令和6年10月改正前の内容（「中学生まで」「所得制限あり」）の
  まま残っており、公式ページと逆の内容になっていたため修正した。**
  検証：設計書の描画チェック（壊れたリンク無し）に加え、`renderConcernSet` を実データで
  実行し、全36件が表示されること・絞り込みで件数が追随すること・該当0件の組み合わせでも
  エラーにならないことを確認。`monitor_stock.py` が新データを読めることも確認。
  ブラウザでの目視確認（タスク10）は未実施。2026-09-17 push 済み。
- 2026-09-17: [Antigravity] 設計書 `design-specs/2026-09-16_child-support-page-renovation.md` に基づき、UI（Antigravity担当）のタスク4a・4b・5〜9を `src/index.html` に実装完了。
  (1) タスク4b（仕様0：全体像セクション）：`child_support_base.json` の `highlights` 配列から冒頭セクションを描画。短い見出し語＋事実の2列（スマホ幅でも崩れないレイアウト）で構成し、各制度カードへのページ内リンク（`#base-{base_id}`）を設定。データが存在しない場合はセクションごと非表示。
  (2) タスク5（仕様1：カテゴリ件数表示）：カテゴリ見出し（h2）の右横に `text-xs text-inkSoft` で重複を除いた実表示件数（`catBaseIds.size`）を添え、プロフィール絞り込み変更時にもリアルタイムで連動。
  (3) タスク6（仕様2：困りごと見出し様式変更）：h3を左に `bay` の縦罫線（`border-l-2 border-bay/40 pl-3`）＋ `text-[15px]` ＋ `font-medium` に変更し、上部余白（`mt-6`）を付与。「子育て支援制度」と「一人暮らし・若者支援」の両タブでスタイルが崩れないことを確認。
  (4) タスク7 & 4a（仕様3 & 3b：カード内情報順の整理とリンク無しカード対応）：カード上部の最終確認バッジを外し、カード最下部の区切り線下（`border-t border-line`）へ移動（3年超警告ボックスは位置維持）。既存の `hasLink` 分岐を維持しつつ、リンク無しカード（学校給食費等）はCTA非表示・出典および「※ この制度は市の公式サイトに専用ページがありません」＋最終確認日を下部に整流化。
  (5) タスク8（仕様4：対象ステージ表示）：種別バッジ横に対象ステージ・世帯状況を表示。全ステージ対象時は「すべての年代」と省略し、`household_type` が空の制度で余計な文字列が出ないことを確認。
  (6) タスク9（仕様5：ジャンプナビのスクロール余白修正）：ヘッダー要素の実高さを計測してCSS変数 `--concern-header-h` / `--ya-header-h` に動的設定し、カードおよびカテゴリ見出しに `.scroll-mt-header` を適用。スマホ幅でプロフィールタグが折り返してもジャンプ先見出しが隠れないよう対応（リサイズ・タブ切り替え連動）。
  実装後、Node.js環境での実データ読み込み・全機能のテストスクリプトを実行し、例外なく意図通り動作することを確認済み。
- 2026-09-16: [Claude Code] 吉野さんの依頼で「子育て支援制度」タブの改装計画を作成し、
  `design-specs/2026-09-16_child-support-page-renovation.md`に設計書として残した。
  改装の主題は、吉野さんの「水俣市で受けられる子育て支援が一覧でき、なんならアピールできる
  場所にしたい」という意図を受け、(1)冒頭に「全体像」セクションを置く、(2)説明文を事実寄りに
  書き直す、(3)掲載制度の件数を増やす、の3点（折りたたみ・検索はスコープ外）。
  2026-08-26に「誘導的・勧誘的な言い回しは避けたい」と決めた経緯があるため、
  **アピールは形容詞ではなく事実の強さで行う**（「子育てしやすい街です」ではなく
  「医療費は18歳まで自己負担なし」と書く）という基準を設計書に明記した。
  市自身の移住サイトが「18歳まで医療費の自己負担がありません」を筆頭の売りにしている一方、
  このサイトの同じ制度の説明文は「自己負担額の一部または全部を助成する制度」となっており、
  一番強い事実が読み取れない状態だった。
  **給食費**については、吉野さんから「令和8年5月の議会だよりに小中学校の無償化が載っている
  はず」との情報を得たが、議会だより第106号は全22ページが画像のスキャンPDF（28MB・フォント0）、
  令和8年度予算概要PDFも日本語フォントにToUnicodeが無く、いずれもテキスト抽出できず
  **一次情報での裏取りが未完了**。国の制度は2026年4月開始・公立小学校のみが対象のため、
  中学校も無償なら市独自の上乗せとなり価値が高い。市の公式サイトには給食費支援の制度ページ自体が
  存在せず（給食カテゴリは給食だより・献立表・食育だよりの3件のみ）、
  「載せたい情報ほど公式サイトに無い」という一次情報主義との衝突が生じている。
  → **同日、吉野さんが議会だより第106号と補正予算書の該当箇所を画像で提供し、事実が確定した。**
  小学校：学校給食費補助事業6,304万円（市内小学校に在籍する児童について給食費の全額を補助、
  国からの交付金で不足する分は市が支出）。中学校：水俣市学校給食費補助事業3,088万円
  （予算書では(款)9教育費(項)5保健体育費(目)5給食施設費・節18として30,885千円、
  説明欄に「水俣市学校給食費補助金」と明記）。**国の制度は公立小学校のみが対象のため、
  中学校分は水俣市独自の措置**であり、アピール材料としての価値が高い。
  補正予算の事業で恒久性が資料から読み取れない点は、吉野さんの判断で
  **`permanence: ongoing`として掲載しつつ年度を必ず明記する**（「給食費は無料です」ではなく
  「令和8年度は全額補助」と書く）方針とした。また市の公式サイトに制度ページが存在しないため、
  **`official_url: null` のカードを許容する**ことを決定し、スキーマに`source_note`（出典名）を
  新設、UI側もCTAボタンを出さず出典を表示する仕様を設計書に追加した。
  なお`monitor_stock.py`は`official_url`のページ更新を監視する仕組みのため、
  **リンクを持たないこの項目は自動監視から外れる**（年度替わりの確認は当面人手で行う）点を
  設計書の「運用上の注意」に記載した。水俣市サイトの子育て関連6カテゴリ
  （list00730/00736/00737/00738/00739/00740）を全件洗い出して現在の27制度と突き合わせた結果、
  **予防接種が1件も載っていない・奨学金が就学援助1件のみ・発達支援の窓口が無い**という3つの穴が
  判明し、追加候補10件を特定した。あわせて、`sick-child-care`・`gakudo-club`・`child-allowance`の
  3件が恒久的な制度説明ページではなく年度限定の告知ページを`official_url`に登録しており、
  年度替わりで内容が変わる構造的リスクがあることも判明（`sick-child-care`は常設ページ
  kiji003384が別に存在するため差し替え候補）。物価高対応子育て応援手当（kiji0034516）は
  `permanence=time_limited`に該当するため意図的に掲載対象外とした。
...
- 2026-08-28: [Claude] 吉野さんから「README.mdが古い」との指摘を受け、内容を現状に合わせて
  ...
  `monitor_stock.py`によるストック情報監視の仕組み（`weekly-monitor.yml`）、プロフィール
  絞り込み機能（ローカルストレージのみで完結）、現在のファイル構成を反映した。
  `SPEC.md`（詳細な現状仕様）・`docs/agent-notes/`（運用メモ）とは役割を分け、README.mdは
  「セットアップ手順＋仕組みの概要」という第三者向け入り口の位置づけを維持している。
- 2026-08-28: [Claude] 吉野さんが別のAIに依頼した法的観点のレビュー（水俣市サイトポリシーとの
  適合性）についてセカンドオピニオンを求められ、実際のコード・水俣市サイトポリシー・robots.txt
  を確認した上で回答した。footer（[src/index.html:199](../../src/index.html#L199)）の
  「データ提供：水俣市」という表記が、市との正式な提携があるかのような誤解を招くおそれがある
  との指摘に同意し、「情報元：水俣市ウェブサイト」に変更した。なお自動収集の頻度配慮
  （1日1回・ソース間1秒ウェイト）は`scripts/update_data.py`に既に実装済みで、READMEにも
  既に明記されていたため追加対応は不要と判断した（水俣市robots.txtにCrawl-delay指定が
  ないことも確認済み）。
- 2026-08-27: [Claude] 吉野さんから「ストック情報（子育て支援制度・一人暮らし若者支援
  両タブ）の『関連する最新のお知らせ』が、あまりピンと来ない結果になっている。
  一度オフにしたい」との相談。トピックタグの緩い一致だけで新着記事を紐付けており、
  精度の粗さが原因と考えられる。この機能（`concernSectionHTML`内の`matched`計算・
  表示、および専用の`newsCardHTML`関数）は他のどこからも参照されておらず独立した
  機能だったため、オフにしても副作用が無いことを確認したうえで完全に削除した
  （空セクション判定からも`matched`の考慮を除去）。子育て支援制度・一人暮らし若者
  支援の両タブが同じ`concernSectionHTML`を共有しているため、1箇所の変更で両方に
  反映される。jsdomで両タブとも「関連する最新のお知らせ」の文言が残っていないこと、
  セクション数（16件・9件）に変化がないことを確認済み。
- 2026-08-26: [Claude] 吉野さんがSPEC.mdを別のClaudeセッションに渡して利用促進戦略を
  検討した結果の引き継ぎメモ「一人暮らし・若者向け支援情報 拡張方針」を受け取り、実装した。
  UI配置は独立した新タブ、着手順は「情報源リサーチから」という吉野さんの指定に従い、
  水俣市の移住・定住促進サイト等をWebFetch/WebSearchで実際に調査し、9件の制度を
  4カテゴリ（住まい／お金・仕事／つながり・くらし／手続き・トラブル）で発見・検証した
  （若年層住宅取得応援奨励金、若者人材確保奨学金返還支援補助金、就業・創業者等転入支援
  奨励金、結婚新生活支援補助金、空き家バンク、空き家リフォーム等補助金、移住定住お試し
  ハウス、消費生活センター、総合相談窓口）。婚活支援は恒常的な制度ページが無く、
  国民健康保険料の減免制度は該当ページを特定できなかったため今回は見送った。
  `child_support_base.json`/`concern_mapping.json`と同じスキーマで
  `young_adult_support_base.json`/`young_adult_concern_mapping.json`を新設
  （child_stage/household_typeは持たず、プロフィール絞り込みの対象外）。
  `src/index.html`に3番目のタブ「一人暮らし・若者支援」を追加し、
  `concernSectionHTML`/`renderConcern`をデータセットを引数で切り替えられる形に汎用化
  （`renderConcernSet`として共通化、既存呼び出し箇所はデフォルト引数で従来通り動作）。
  新タブの初期化は独立したtry/catchにし、失敗しても他タブに影響しないようにした
  （2026-08-24のprofile-tags要素欠落による全タブクラッシュの教訓を反映）。
  jsdomで3タブすべての切り替え・9件の困りごと・4カテゴリの表示を検証済み。
- 2026-08-26: [Claude] 吉野さんから「このページの仕様書を作ってほしい。別のClaude
  セッションに利用促進戦略を考えてもらう予定」との依頼を受け、リポジトリ直下に
  `SPEC.md` を新規作成。サイトの概要・2タブの情報構成・データ/自動化の仕組み・
  現状の数値スナップショット（life_info.json 193件、child_support_base.json
  27制度等）・強み・弱み（子育て限定であることや「全世代」タグが7割を占める実態
  など）・戦略検討のための論点をまとめた。`docs/agent-notes/CURRENT_STATUS.md`
  （運用状況の逐次更新用）とは役割を分け、`SPEC.md`は都度上書きするスナップショット
  文書として位置づける。
- 2026-08-26: [Claude] 吉野さんから「使える制度をさがす」タブの見せ方について相談。
  当初「水俣に来たらこんな支援が受けられますよ」という誘導的なコピー案
  （タブ名「水俣で受けられる支援」等）を提示したが、吉野さんから「ニュアンスは
  現在のものの方が良い。誘導するような文言は省きたい」とフィードバックがあり、
  最小限の言い換えに変更した。`src/index.html`の3箇所
  （タブボタン・`#view-concern`のh2見出し・リード文）で、「〜が使える制度を探す」
  という利用者目線の表現から、「水俣市の子育て支援制度」という中立的な紹介表現に
  変更（「使える制度をさがす」→「子育て支援制度」（タブ）／「水俣市の子育て支援制度」
  （見出し）。呼びかけ・勧誘の言葉（「〜たら」「〜ますよ」等）は使っていない。
  制度カード自体の説明文（`child_support_base.json`）は変更していない。
- 2026-08-25: [Claude] 吉野さんから「確認日より実際の更新日がはっきりしているものを
  優先的に上げたい」との要望。単純に日付だけで混ぜて並べると、`collected_at`
  （確認日、常にほぼ「今」の値）が実日付より不当に有利になる問題があった一方、
  実日付グループを無条件に優先すると2018年の古い記事が今日の新着候補より
  上位に来る副作用があることを実データ（実日付は2018-03-05〜2026-08-25の93件、
  確認日のみが98件）で説明した上で、吉野さんの希望により**実日付グループを
  確認日グループより常に優先する二段階ソート**を試験導入することにした。
  `update_data.py`の`sort_key()`を`(実日付なら1/確認日のみなら0, 日付文字列)`の
  タプルを返す形に変更し、`src/index.html`のフロントエンド側ソートも同じロジックに
  揃えた。既存の`life_info.json`にも遡って適用し、実データで意図通りの順序
  （2026年の実日付→...→2018年の実日付→確認日グループ）になることを確認済み。
  ただし副作用（古い実日付コンテンツが目立つ可能性）は未評価のため、
  実際の見た目次第では別方式に戻す可能性がある旨をCURRENT_STATUS.mdに明記した。
